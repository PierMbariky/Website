...
const LessonPages = () => {
    const { lessonId, unitId } = useParams(); // Extract both lessonId and unitId from URL
    ...
    const markLessonAsCompleted = () => {
        if (email) {
            fetch(`${ipAddress}/api/lessons/${lessonId}/complete`, {
                ...
                body: JSON.stringify({ unitId, lessonId, email }), // Include both unitId and lessonId in the request body
            })
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
            ...
            // If the user is not logged in, save the completion status in local storage
